...
public class ProjectionToProjectorVisitorTest extends CrateUnitTest {
    ...
    public void testGroupProjector() throws Exception {
        ...
        List<Aggregation> aggregations = Arrays.asList(
            Aggregation.finalAggregation(avgInfo, Collections.singletonList(new InputColumn(1, DataTypes.DOUBLE)), Aggregation.Step.ITER),
            Aggregation.finalAggregation(countInfo, Collections.singletonList(new InputColumn(0, DataTypes.STRING)), Aggregation.Step.ITER)
        );
